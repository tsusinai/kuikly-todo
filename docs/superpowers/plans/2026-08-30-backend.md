...
- **腾讯 `~` 字段位号**(已对照真实响应核对):`name(1)/code(2)/price(3)/open(5)/change(31)/changePct(32)/high(33)/low(34)/pe(39,A股与HK均为39;40恒为空)/floatCap(44,单位亿)/marketCap(45,单位亿)`。HK 与 A股 在 31..45 位号一致(pe 同为 39,40 恒为空)。**切勿用 change(21)/changePct(22)/high(23)/low(24)**——那些是买卖五档。
...
object QuoteParser {
    ...
    const val LOW = 34
    const val PE_A = 39
    const val PE_HK = 39   // 实测 hk PE 也在 39,40 恒为空
