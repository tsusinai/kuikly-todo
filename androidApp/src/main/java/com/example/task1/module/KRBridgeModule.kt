package com.example.task1.module
...
import com.example.task1.KRApplication
import com.example.task1.KuiklyRenderActivity
import com.example.task1.adapter.KRHapticAdapter
...
class KRBridgeModule : KuiklyRenderBaseModule() {
    ...
    // 触觉反馈：短震。解析 type 后转发到 Android 独立实现（adapter/KRHapticAdapter）。
    private fun vibrateShort(params: String?) {
        if (params == null) {
            return
        }
        KRHapticAdapter.vibrate(activity, JSONObject(params).optString("type", "heavy"))
    }
